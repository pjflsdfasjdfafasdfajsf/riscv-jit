...
pub fn build(b: *std.Build) void {
    ...
    host.root_module.linkLibrary(raylib_dep.artifact("raylib"));
    ...
    const guest = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            .optimize = optimize,
            // let's not strip our update function
            .strip = false,
        }),
    });
    // must be paired with `.strip = false` above!
    guest.entry = .{ .symbol_name = "update" };
